...
HGCalCluster::ClusterWord HGCalCluster::formatFirstWord( const ClusterAlgoConfig& config ) {
  ...
  // ClusterWord clusterWord = TTBV(hw_Et + hw_EtEgamma + hw_GCTSelectBits + hw_fracCEE + hw_fracCEECore + hw_fracCEHFront + hw_firstLayerZ + hw_spare).bs();
...
HGCalCluster::ClusterWord HGCalCluster::formatSecondWord( const ClusterAlgoConfig& config ) {
  ...
  // std::cout << "Phi : " << 1.0 * wphi() / w() << " " << Mean_phi_Quotient() << " " << Mean_phi_Fraction() << std::endl;
