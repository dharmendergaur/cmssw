...
private:
  ...
  std::unordered_map<unsigned, signed> stage1links_whichsector_;
...
HGCalTriggerGeometryBase::geom_set HGCalTriggerGeometryV9Imp4::getStage1FpgasFromStage2Fpga(
  ...
  geom_set stage1_ids;
  std::cout << "In getStage1FpgasFromStage2Fpga with ID : " << stage2_id << std::endl;
...
HGCalTriggerGeometryBase::geom_set HGCalTriggerGeometryV9Imp4::getStage2FpgasFromStage1Fpga(
  ...
  std::cout << "Getting S1 links from S1 FPGA" << std::endl;
  geom_set stage1_links = getStage1LinksFromStage1Fpga(stage1_id);
  std::cout << "Looping S1 links, getting S2 FPGAs" << std::endl;
  for (const auto& stage1_link : stage1_links) {
...
HGCalTriggerGeometryBase::geom_set HGCalTriggerGeometryV9Imp4::getStage1LinksFromStage2Fpga(
  ...
  for (auto stage2_itr = stage2_itrs.first; stage2_itr != stage2_itrs.second; stage2_itr++) {
    ...
    // }
    const auto whichSector = stage1links_whichsector_.at(label);
  ...
  std::cout << "Outputting : " << stage1link_ids.size() << std::endl;
...
unsigned HGCalTriggerGeometryV9Imp4::getStage2FpgaFromStage1Link(const unsigned link_id) const {
  HGCalTriggerBackendDetId id(link_id);
  std::cout << "Getting Stage 2 FFPGA for S1 link : " << id.label() << std::endl;
  bool same_sector = stage1link_to_stage2_.at(id.label());
  std::cout << "Same sector var : " << stage1link_to_stage2_.at(id.label()) << " " << same_sector << std::endl;
...
HGCalTriggerGeometryBase::geom_set HGCalTriggerGeometryV9Imp4::getStage1LinksFromStage1Fpga(
  ...
  std::cout << "Getting links for s1 id : " << id.label() << std::endl;
  ...
  std::cout << "Got S1 links : " << stage1link_ids.size() << std::endl;
  return stage1link_ids;
}
...
HGCalTriggerGeometryBase::geom_set HGCalTriggerGeometryV9Imp4::getModulesFromLpgbt(const unsigned lpgbt_id) const {
  ...
  for (auto lpgbt_itr = lpgbt_itrs.first; lpgbt_itr != lpgbt_itrs.second; lpgbt_itr++) {
    ...
    // const auto moduleID = HGCalTriggerModuleDetId(
    //     HGCalTriggerSubdetector(subdet), id.zside(), type, layer, id.sector(), moduleU, moduleV);
    // std::cout << "Module : " << moduleID.rawId() << std::endl;
  }
  return modules;
}
...
void HGCalTriggerGeometryV9Imp4::fillMapsFromXML() {
  std::cout << "xmlxmlxmlxmlxmlxmlxmlxmlxmlxmlxmlxmlxml" << std::endl;
  std::cout << "In fillMapsFromXML" << std::endl;
  std::cout << "xmlxmlxmlxmlxmlxmlxmlxmlxmlxmlxmlxmlxml" << std::endl;
  ...
  l1thgcmapping::OpenBackendMapping( "scenarios/CombinedTD.60.MixedTypes.NoSplit/BackendMapping.xml" , lMaps ); 
    ...
    std::cout << "N links : " << nBELinksToS2In << std::endl;
    ...
    for (const auto& [belink, s1out]: lMaps.belink_to_s1out) {
      ...
      if ( endcap == 0 && s2tm == 0 && s1Sector == 0 ) {
        ...
        //  Could we use belink raw id as the link id here instead of definig a simpler 0->N counter?
        const auto link_id = belink; // l1thgcmapping::get_io_index(s1out) + s1Subsector*3 + s1Index*6;
        stage1link_to_stage1_.emplace(iLink,s1Index);
        // Double check that we can do reciprocal map (i.e. opposite of stage1link_to_stage1_) at this point
        stage1_to_stage1links_.emplace(s1Index, iLink);
        ...
        std::cout << "Adding to stage1link_to_stage2_ : " << iLink << " " << link_id << " " << whichSector << std::endl;
        stage1link_to_stage2_.emplace( iLink, whichSector );
        ++iLink;
      }
    }
    ...
    for (const auto& [s1, region]: lMaps.stage1_to_region) {
      ...
      for (auto it = motherboardRange.first; it != motherboardRange.second; ++it) {
        ...
        // unsigned nLPGBTs = lMaps.motherboard_to_nTrigLPGBT.at(it->second);
        // stage1_to_nLpgbts_.emplace( s1Index, nLPGBTs);
        ...
        // for ( unsigned int i=0; i<nLPGBTs; ++i ) {
        //   uint32_t lpgbt_id = (it->second | i);
        //   lpgbt_id_vec.push_back(lpgbt_id);
        // }
        // stage1_to_lpgbts_.emplace(s1Index, lpgbt_id_vec);
      }
      // if ( s1Index == 2 ) {
      //   nlpgbts += lpgbts.size();
      //   std::cout << "Adding lpgbt to s1 2 : ";
    ...
    // std::cout << "N lpgbts added to s1 2 : " << nlpgbts << std::endl;
    ...
    for (const auto& [motherboard, region]: lMaps.motherboard_to_region) {
      ...
      // for ( unsigned int i=0; i<nLPGBTs; ++i ) {
      //   uint32_t lpgbt_id = (motherboard | i);
      //   lpgbt_to_stage1_.emplace(lpgbt_id,s1Index);
      // }
      ...
      for (auto it = moduleRange.first; it != moduleRange.second; ++it) {
        ...
        if ( ( ( module_uv.first == 1 && module_uv.second == 5 ) || ( module_uv.first == 7 && module_uv.second == 7 ) ) && layer == 7 ) {
          std::cout << "Interesting module : " << layer << " " << subdetId << " " << module_uv.first << " " << module_uv.second << std::endl;
          std::cout << "Stage 1 index : " << s1Index << std::endl;
          std::cout << "Packed value : " << packed_value << std::endl;
        }

        for ( const auto lpgbt : lpgbtIds ) {
          if ( ( ( module_uv.first == 1 && module_uv.second == 5 ) || ( module_uv.first == 7 && module_uv.second == 7 ) ) && layer == 7 ) {
            std::cout << "LPGBT for module : " << lpgbt << std::endl;
          }
        ...
        // for ( unsigned int i=0; i<nLPGBTs; ++i ) {
        //   uint32_t lpgbt_id = (motherboard | i);
        //   std::cout << "Module, LPGBT ID : " << packed_value << " " << motherboard << " " << uint16_t(motherboard & 0xFFFF) << " " << lpgbt_id << " " << uint16_t(lpgbt_id & 0xFFFF) << std::endl;
        //   lpgbt_to_modules_.emplace(lpgbt_id,packed_value);
        //   module_to_lpgbts_.emplace(packed_value,lpgbt_id);
        // }
        module_to_stage1_.emplace(packed_value, s1Index);
        links_per_module_.emplace(packed_value, 0); // dummy
      }
    }
    std::cout << "Max motherboard : " << maxMBID << std::endl;
    ...
    for (unsigned lpgbt_id = 0; lpgbt_id < mapping_config.at("lpgbt").size(); lpgbt_id++) {
      ...
      for (auto& modules : mapping_config.at("lpgbt").at(lpgbt_id).at("Modules")) {
        ...
        if (result.second == false &&
          ...
          edm::LogError("HGCalTriggerGeometryV9Imp4") << "One module is connected to two separate Stage1 FPGAs";
        }
      }
    }
