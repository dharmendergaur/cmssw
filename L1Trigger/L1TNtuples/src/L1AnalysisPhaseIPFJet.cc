...
// void L1Analysis::L1AnalysisPhaseIPFJet::SetGenJet(const edm::Handle<reco::GenJetCollection> genJets, const reco::JetFlavourInfoMatchingCollection& genJetFlavours, unsigned maxL1Extra)
void L1Analysis::L1AnalysisPhaseIPFJet::SetGenJet(const edm::Handle<reco::GenJetCollection> genJets, unsigned maxL1Extra)
{
  ...
  for( ; genJetItr != genJetEnd ; ++genJetItr) {
    ...
    int partonFlavour = -1;
    int hadronFlavour = -1;
    // for (const reco::JetFlavourInfoMatching& jetFlavourInfoMatching : genJetFlavours) {
    //   if (deltaR(genJetItr->p4(), jetFlavourInfoMatching.first->p4()) < 0.4) {
    //     partonFlavour = jetFlavourInfoMatching.second.getPartonFlavour();
    //     hadronFlavour = jetFlavourInfoMatching.second.getHadronFlavour();
    //     break;
    //   }
    // }
    l1extra_.genJetPartonFlavour.push_back(partonFlavour);
    l1extra_.genJetHadronFlavour.push_back(hadronFlavour);
  }
...
void L1Analysis::L1AnalysisPhaseIPFJet::SetCNNScore(const edm::Handle<double> cnnScore)
{
  l1extra_.cnnScore = *cnnScore;
}
