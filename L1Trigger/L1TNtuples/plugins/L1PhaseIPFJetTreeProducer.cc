...
private:
  ...
  edm::EDGetTokenT<double>cnnScore_;
...
L1PhaseIPFJetTreeProducer::L1PhaseIPFJetTreeProducer(const edm::ParameterSet& iConfig){
  ...
  cnnScore_ = consumes<double>(iConfig.getUntrackedParameter<edm::InputTag>("cnnScore"));
...
L1PhaseIPFJetTreeProducer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
 ...
 l1Extra->Reset();
  ...
  if (genJets.isValid()){ 
    ...
    // l1Extra->SetGenJet(genJets,jetFlavourInfosProd,maxL1Extra_);
  } else {
  ...
  edm::Handle<double> cnnScoreHandle;
  iEvent.getByToken(cnnScore_,cnnScoreHandle);
  // std::cout<< *cnnScoreHandle << std::endl;
  if (cnnScoreHandle.isValid()){
  l1Extra->SetCNNScore(cnnScoreHandle);
  } else{
    edm::LogWarning("MissingProduct") << "CNN scores not found. Branch will not be filled" << std::endl;
  }
  ...
  if (phaseIL1PFMET.isValid()){
    ...
    edm::LogWarning("MissingProduct") << "L1PhaseIPFJet PFSums MET not found. Branch will not be filled" << std::endl;
  }
  ...
  if (phaseIL1PFJetSums.isValid()){
    ...
    edm::LogWarning("MissingProduct") << "L1PhaseIPFJet PFSums HT/MHT not found. Branch will not be filled" << std::endl;
  }
