...
namespace l1thgcfirmware {
    void decodeClusters(const std::array<std::vector<ap_uint<64>>, 4>& frames) {
        std::cout << "In decode Clusters" << std::endl;

        unsigned nFrames = frames.at(0).size();
        for ( unsigned int iFrame=0; iFrame < nFrames; ++iFrame ) {
            const auto clusterWord_0 = frames.at(0).at(iFrame);
            const auto clusterWord_1 = frames.at(1).at(iFrame);
            const auto clusterWord_2 = frames.at(2).at(iFrame);
            const auto clusterWord_3 = frames.at(3).at(iFrame);

            if ( clusterWord_0 != 0 ) {
                std::cout << "iFrame : " << iFrame << std::endl;
                std::cout << "Cluster words : " << clusterWord_0.to_string() << " " << clusterWord_1.to_string() << " " << clusterWord_2.to_string() << " " << clusterWord_3.to_string() << std::endl;
                decodeFirstWord( clusterWord_0 );
                decodeSecondWord( clusterWord_1 );
                decodeThirdWord( clusterWord_2 );
                decodeFourthWord( clusterWord_3 );
